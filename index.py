...
# Realizar la solicitud GET con los encabezados proporcionados
...
if response.status_code == 200:
    ...
    mpd_url = data['PlaybackDetails'][0]['ManifestUrl']

    # Construir la URL final con el token
    final_mpd_url = f"{mpd_url}?p=web&dazn-token={token}"

    # Imprimir el token y la URL del MPD
    print(f"Token: {token}")
    print(f"MPD URL: {final_mpd_url}")

else:
    # Imprimir mensaje de error si la solicitud no fue exitosa
    print(f"Error al obtener el contenido de la URL. Código de estado: {response.status_code}")
